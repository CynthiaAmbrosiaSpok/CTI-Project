...
public class PhoneControlPage extends PhoneControlLocators {
	...
	public void clickAnswerButton(String phoneNumber) {
		...
		List<String> buttonList = Arrays.asList(phoneNumber.split(","));
		for (int i = 0; i < buttonList.size(); i++) {
//			eDriver.findElement(By.name(buttonList.get(i).strip())).click();
			eDriver.findElement(By.name(buttonList.get(i))).click();
		}
		
		//Click 'Cancel' to exit any attempts to perform an outgoing call (by clicking a button that does not have an incoming call)
		cancelButton.click();
	}
...
//			softAssert.assertEquals(failedConnectionMessage.getText(), "established");
		...
		try {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyApplicationConnection");
		}
	...
	public SoftAssert verifyLogoutStatus(SoftAssert softAssert) {
		...
		if (loginButton.isEnabled() && !logoutButton.isEnabled()) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyLogoutStatus");
		}
	...
	public SoftAssert verifyAgentStatus(SoftAssert softAssert, String agentStatus) {
		...
		if (agentStatus.equalsIgnoreCase("ready")) {
			...
			if (readyButton.getAttribute("Name").equals("♦ Ready")) {
				...
				reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyAgentStatus");
			}
	...
	public SoftAssert verifySuccessfulLogin(SoftAssert softAssert) {
		...
		if (notReadyButton.getAttribute("Name").equals("♦ Not-Ready") && !loginButton.isEnabled() && logoutButton.isEnabled()) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifySuccessfulLogin");
		}
	...
	public SoftAssert verifyLoginStatus(SoftAssert softAssert) {
		...
		if (!loginButton.isEnabled() && logoutButton.isEnabled()) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyLoginStatus");
		}
	...
	public SoftAssert verifyIncomingCall(SoftAssert softAssert, String ozekiNumber, String phoneNumber) {
		...
		try {
			//Initialize Variable(s)
			successMessage = "Spok CTI Client: The incoming call is correctly set to: '" + ozekiNumber + " to " + phoneNumber + "'";
			failureMessage = "Spok CTI Client: The incoming call is incorrectly set to: '" + displayInfoField1.getText() + "', instead of '" + ozekiNumber + " to " + phoneNumber + "'";
		} catch (Exception e) {
			try {
				//Initialize Variable(s)
				successMessage = "Spok CTI Client: The incoming call is correctly set to: '" + ozekiNumber + " to " + phoneNumber + "'";
		...
		if (displayInfoField1.getText().equalsIgnoreCase(ozekiNumber + " to " + phoneNumber)) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyIncomingCall");
		}
	...
	public SoftAssert verifyAnsweredCall(SoftAssert softAssert) {
		...
		if (callStateField.get(0).getText().equalsIgnoreCase("connected")) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyAnsweredCall");
		}
	...
	public SoftAssert verifyOutgoingCall(SoftAssert softAssert) {
		...
		if (callStateField.get(0).getText().equalsIgnoreCase("ringback")) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyOutgoingCall");
		}
	...
	public SoftAssert verifyHoldCall(SoftAssert softAssert) {
		...
		if (callStateField.get(0).getText().equalsIgnoreCase("hold")) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyHoldCall");
		}
	...
	public SoftAssert verifyTransferredCall(SoftAssert softAssert) {
		...
		if (callStateField.get(0).getText().equalsIgnoreCase("soft_hold") && callStateField.get(1).getText().equalsIgnoreCase("ringback")) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyTransferredCall");
		}
	...
	public SoftAssert verifyOutgoingConferenceCall(SoftAssert softAssert) {
		...
		if (callStateField.get(0).getText().equalsIgnoreCase("soft_hold") && callStateField.get(1).getText().equalsIgnoreCase("ringback")) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyOutgoingConferenceCall");
		}
	...
	public SoftAssert verifyAcceptedConferenceCall(SoftAssert softAssert) {
		...
		if (callStateField.get(0).getText().equalsIgnoreCase("soft_hold") && callStateField.get(1).getText().equalsIgnoreCase("connected")) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyAcceptedConferenceCall");
		}
	...
	public SoftAssert verifySuccessfulConferenceCall(SoftAssert softAssert) {
		...
		if (callStateField.get(0).getText().equalsIgnoreCase("connected")) {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifySuccessfulConferenceCall");
		}
	...
	public SoftAssert verifyReleasedCall(SoftAssert softAssert) {
		...
		try {
			...
			reportLoggerMethods.reportFailedCheckpoint("Failed: " + failureMessage, "PhoneControlPage_verifyReleasedCall");
		} catch (Exception e) {
